...
public class SelectKthSmallest {
	 ...
	 * 
	 * While QuickSort time complexity is O(n*log(n)), SelectKthSmallest QuickSelect algorithm time
	 * complexity is only O(n) because we don't have to sort the other partitions of the array,
	 * partitioning has time complexity of O(n) and Knuth Shuffle algorithm's time complexity is
	 * also O(n).
	 * 
	 * The ineffective solution is to sort an array first in O(n*log(n)), then select k-th smallest
	 * element in one scan, which results in O(n*log(n) + n) = O(n*log(n)) runtime.
	 */
	...
	public static <T extends Comparable<T>> T quickSelect(T[] a, int k) {
		...
		if (k < 0 || k >= a.length) {
			throw new IndexOutOfBoundsException(
					"Index of the element to be selected is out of bounds");
		}

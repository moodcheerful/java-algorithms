 ...
 *  
 * Given a sequence	{x1, ..., xn} of numbers one-by-one from a data stream,
 * find at each step i the median of {x1, ..., xi}.
 * 
 * Time Complexity: O(log(i)) at each step i.
 * 
 * Solution: maintain 2 heaps, one that supports Extract-Min operation and one that supports Extract-Max,
 * and maintain invariant that maxHeap on the left half holds elements that are less than the effective median, 
 * and minHeap on the right half holds elements that are greater than the effective median.
 * 
 */
